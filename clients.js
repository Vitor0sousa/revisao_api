...
// GET: Obter um único cliente pelo ID
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ client_id: Number(result.insertId), message: 'Cliente criado com sucesso' });
    } catch (error) {
...
router.put('/:id', async (req, res) => {
    const { name, email, address } = req.body;
    try {
        const result = await pool.query('UPDATE clients SET name = ?, email = ?, address = ? WHERE client_id = ?', [name, email, address, req.params.id]);
